function Letters_and_Backgrounds () {
    Gray = sprites.create(assets.image`Gray`, SpriteKind.Player)
    ...
    Green = sprites.create(img`
        ...
        7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 
        `, SpriteKind.Player)
    A = sprites.create(assets.image`A`, SpriteKind.Player)
    B = sprites.create(assets.image`B`, SpriteKind.Player)
    C = sprites.create(assets.image`C`, SpriteKind.Player)
    D = sprites.create(assets.image`D`, SpriteKind.Player)
    E = sprites.create(assets.image`E`, SpriteKind.Player)
    F = sprites.create(assets.image`F`, SpriteKind.Player)
    G = sprites.create(assets.image`G`, SpriteKind.Player)
    H = sprites.create(assets.image`H`, SpriteKind.Player)
    I = sprites.create(assets.image`I`, SpriteKind.Player)
    J = sprites.create(assets.image`J`, SpriteKind.Player)
    K = sprites.create(assets.image`K`, SpriteKind.Player)
    L = sprites.create(assets.image`L`, SpriteKind.Player)
    M = sprites.create(assets.image`M`, SpriteKind.Player)
    N = sprites.create(assets.image`N`, SpriteKind.Player)
    O = sprites.create(assets.image`O`, SpriteKind.Player)
    P = sprites.create(assets.image`P`, SpriteKind.Player)
...
function GameLoop () {
    ...
    if (!(True_Word)) {
        game.splash("That's not a word.")
    } else {
        Set_Guess()
        _Guesses += 1
        game.showLongText("Press 'A' to guess.", DialogLayout.Right)
    }
}
function SetBoard () {
    Gray.setPosition(-8, 10)
    Proj_Y = 10
    for (let index = 0; index < 6; index++) {
        ...
        for (let index = 0; index < 5; index++) {
            ...
            projectile = sprites.createProjectileFromSprite(img`
                ...
                d d d d d d d d d d d d d d d d 
                `, Gray, 0, 0)
            projectile.setPosition(Proj_X, Proj_Y)
        }
        Proj_Y += 19
    }
}
...
let Proj_X = 0
let word_as_list: string[] = []
let Proj_Y = 0
let wordString = ""
let wordList: string[] = []
let WordFromLetterList = ""
let createWordIndex = 0
let createListIndex = 0
let letterList: string[] = []
let Sprite_Array: Sprite[] = []
let Z: Sprite = null
let Y: Sprite = null
let X: Sprite = null
let W: Sprite = null
...
let _Guesses = 0
let Secret_Word = ""
game.showLongText("Press 'A' and wait while your wordle loads.", DialogLayout.Top)
Letters_and_Backgrounds()
Create_List_of_Words()
Secret_Word = "stick"
// Remove before submitting code.
game.showLongText(Secret_Word, DialogLayout.Bottom)
Word_As_List()
SetBoard()
_Guesses = 0
let Win = false
pause(2000)
while (_Guesses < 6 && !(Win)) {
    GameLoop()
}
